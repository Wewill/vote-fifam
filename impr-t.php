...
$rep=$cbdd->query('SELECT * FROM tickets WHERE snc="'.$_GET['s'].'" AND val=1 ORDER BY nno ');
	...
	if ($tr%2==0) {$pls=0;$pdf->Line( 0,floor(($tr+2)/2)*148.5,10,floor(($tr+2)/2)*148.5);} else {$pls=105;$pdf->Line(200,floor(($tr+2)/2)*148.5,210,floor(($tr+2)/2)*148.5);}
		...
		$pdf->Cell(40,7,'vote num�rique :',0,1,'C');
		...
		$pdf->SetFont('Arial','',9);
		$pdf->Cell(85,11,'Entourez votre note (5 �tant la meilleure)',0,1,'C');
		$pdf->SetFont('Arial','I',8);
		$pdf->SetX(10+$pls);
		$pdf->MultiCell(85, 4, "Deux personnes seront tir�es au sort et gagneront une carte Licorne pour la 45e �dition du Festival international du film d�Amiens !");
		$pdf->SetFont('Arial','B',10);
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'Nom : _____________________________________',0,1,'L');
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'Pr�nom : __________________________________',0,1,'L');
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'T�l�phone : ________________________________',0,1,'L');
		$pdf->SetX(10+$pls);
