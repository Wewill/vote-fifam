...
$rep=$cbdd->query('SELECT * FROM tickets WHERE snc="'.$_GET['s'].'" AND val=1 ORDER BY nno ');
	...
	if ($tr%2==0) {$pls=0;$pdf->Line( 0,floor(($tr+2)/2)*148.5,10,floor(($tr+2)/2)*148.5);} else {$pls=105;$pdf->Line(200,floor(($tr+2)/2)*148.5,210,floor(($tr+2)/2)*148.5);}
		...
		$pdf->Cell(40,10,'Prix du public',0,1,'C');
		$pdf->SetX(59.5+$pls);
		$pdf->SetFont('Arial','I',10);
		$pdf->Cell(40,7,'vote numérique :',0,1,'C');
		$pdf->SetXY(10+$pls,(floor($tr/2)*148.5+60));
		$pdf->SetFont('Arial','B',12);
		$pdf->Cell(85,5,''.utf8_decode($nom).'',0,1,'C');
		$pdf->SetX(10+$pls);
		$pdf->SetFont('Arial','I',10);
		$pdf->Cell(85,5,'de '.utf8_decode($ral).' ('.$ann.')',0,1,'C');
		...
		$pdf->SetFont('Arial','',9);
		$pdf->Cell(85,11,'Entourez votre note (5 étant la meilleure)',0,1,'C');
		$pdf->SetFont('Arial','I',8);
		$pdf->SetX(10+$pls);
		$pdf->MultiCell(85, 4, "Deux personnes seront tirées au sort et gagneront une carte Licorne pour la 45e édition du Festival International du Film d'Amiens !");
		$pdf->SetFont('Arial','B',10);
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'Nom : _____________________________________',0,1,'L');
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'Prénom : __________________________________',0,1,'L');
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'Téléphone : ________________________________',0,1,'L');
		$pdf->SetX(10+$pls);
		$pdf->Cell(85,7,'E-mail : ____________________________________',0,1,'L');
